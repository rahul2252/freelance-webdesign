...
export default function Footer() {
  return (
    <footer className="bg-ink text-cream/80 px-6 lg:px-10 py-14">
        ...
        <nav className="flex flex-col gap-2">
          {clientConfig.nav.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="text-sm uppercase tracking-[0.12em] hover:text-accent transition-colors w-fit"
            >
              {item.label}
            </a>
          ))}
      ...
      <div className="mx-auto max-w-7xl mt-12 pt-6 border-t border-cream/15 text-xs text-cream/50">
