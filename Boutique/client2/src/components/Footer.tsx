...
export default function Footer() {
  return (
    <footer className="bg-cream border-t border-moss/15 text-ink/70 px-6 lg:px-10 py-14">
      <div className="mx-auto max-w-7xl grid gap-10 md:grid-cols-3">
        <div>
          <p className="font-serif text-2xl text-ink mb-3 tracking-[0.08em]">
        ...
        <nav className="flex flex-col gap-2">
          {clientConfig.nav.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="link-underline text-xs uppercase tracking-[0.15em] hover:text-moss transition-colors w-fit"
            >
              {item.label}
            </a>
          ))}
      ...
      <div className="mx-auto max-w-7xl mt-12 pt-6 border-t border-moss/15 text-xs text-ink/40">
