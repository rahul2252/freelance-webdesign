  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-colors duration-500 ${
        scrolled ? "bg-cream/90 backdrop-blur-sm border-b border-moss/15" : "bg-transparent"
      }`}
    >
        ...
        <nav className="hidden md:flex items-center gap-10">
          {clientConfig.nav.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className={`link-underline text-xs uppercase tracking-[0.2em] transition-colors hover:text-moss ${
        ...
        <div className="flex items-center gap-4">
          <a
            ...
            className={`hidden sm:inline-flex items-center justify-center w-10 h-10 rounded-full border transition-colors ${
              scrolled
                ? "border-moss/30 text-moss hover:border-moss hover:text-moss-soft"
                : "border-cream/40 text-cream hover:border-cream hover:text-cream/80"
            }`}
          >
      ...
      {open && (
        <nav className="md:hidden bg-cream border-t border-moss/15 px-6 py-6 flex flex-col gap-5">
          {clientConfig.nav.map((item) => (
            <a
              key={item.href}
              href={item.href}
              onClick={() => setOpen(false)}
              className="text-xs uppercase tracking-[0.2em] text-ink/80 hover:text-moss transition-colors"
            >
              {item.label}
            </a>
          ))}
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-moss"
          >
            <WhatsAppIcon className="w-5 h-5" />
            WhatsApp
          </a>
