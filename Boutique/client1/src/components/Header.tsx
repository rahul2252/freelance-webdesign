...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-cream/90 backdrop-blur-sm border-b border-stone/60">
      <div className="mx-auto max-w-7xl px-6 lg:px-10 flex items-center justify-between h-16 md:h-20">
        <a
          href="#home"
          className="font-serif text-2xl md:text-3xl tracking-wide text-ink"
        >
        ...
        <nav className="hidden md:flex items-center gap-10">
          {clientConfig.nav.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="text-sm uppercase tracking-[0.15em] text-charcoal hover:text-accent transition-colors"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-4">
          <a
            ...
            className="hidden sm:inline-flex items-center justify-center w-10 h-10 rounded-full border border-charcoal/20 text-charcoal hover:text-accent hover:border-accent transition-colors"
          >
      ...
      {open && (
        <nav className="md:hidden bg-cream border-t border-stone/60 px-6 py-6 flex flex-col gap-5">
          {clientConfig.nav.map((item) => (
            <a
              key={item.href}
              href={item.href}
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 text-sm uppercase tracking-[0.15em] text-accent"
          >
            <WhatsAppIcon className="w-5 h-5" />
            WhatsApp
          </a>
