...
export default function Booking() {
  const { label, heading, body, ctaLabel, note } = clientConfig.booking;

  return (
    <section className="py-24 md:py-28 px-6 lg:px-8 text-center border-t border-border">
      <Reveal className="max-w-lg mx-auto">
        <p className="text-[10px] uppercase tracking-[0.22em] text-crimson mb-3">{label}</p>
        <h2 className="font-serif text-3xl md:text-4xl text-ink mb-4">{heading}</h2>
        <p className="text-sm text-muted leading-relaxed mb-8">{body}</p>
        <a
          href={clientConfig.whatsapp.link}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2.5 px-9 py-4 bg-[#25D366] text-white text-sm font-medium tracking-[0.03em] hover:brightness-95 transition-[filter]"
        >
