...
export default function Testimonials() {
  const { label, heading, headingAccent, items } = clientConfig.testimonials;

  return (
    <section className="bg-blush border-y border-border py-20 md:py-24 px-6 lg:px-8">
          ...
          {items.map((item, i) => (
            <Reveal key={item.author} delay={i * 100}>
              <div className="bg-white border border-border p-6 h-full flex flex-col">
                <div className="text-mauve text-sm tracking-[3px] mb-2.5">★★★★★</div>
                <p className="text-[13px] text-muted italic leading-relaxed mb-4 flex-1">
                  &ldquo;{item.quote}&rdquo;
                </p>
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-full bg-blush flex items-center justify-center text-xs font-medium text-deep shrink-0">
                    {item.initials}
                  </div>
                  <div>
